...
from bubble_plot import (Occurence, SplitXAxis, Bubble, Facets, BubblePlot,
                         Config, CSVWriter, compute_occurences_from,
...
class TestSplitXAxis(TestCase):
    ...
    def test_update_unkown_facet(self):
        ...
        self.assertRaisesRegex(KeyError, regex, self.x_axis.update,
                               entry, '2020', 'Y')
...
class TestCSVWriter(TestCase):
    def setUp(self):
        self.conf = Config(1, 2, 'year',
                           ['iy', 'ix', 'nbr', 'year', 'y', 'x'])
        self.years = {'2018', '2019', '2020'}
        self.entries = [{'Y': '0', 'X_left': '1',
                         'X_right': '2', 'year': '2018'},
                        {'Y': '3', 'X_left': '4',
                         'X_right': '5', 'year': '2020'},
                        {'Y': '6', 'X_left': '7',
    ...
    def test_prepared_bubbles_data(self):
        prepared_data = self.writer.prepared_bubbles_data(self.x_mapping,
                                                          self.y_mapping,
                                                          self.year_mapping)
...
class TestAPI(TestCase):
    ...
    def test_compute_occurences_from(self):
        ...
        plot, years = compute_occurences_from(self.entries,
                                              self.plot_plan,
                                              self.conf)
    ...
    def test_build_and_save_plots(self):
        ...
        mock = mock_open()
        with patch('bubble_plot.open', mock):
            build_and_save_plots(self.entries,
                                 [self.plot_plan] * number_plot,
                                 self.conf)
